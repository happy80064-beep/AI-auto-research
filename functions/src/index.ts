...
const getAiClient = () => {
    ...
    if (!apiKey) {
        throw new functions.https.HttpsError('failed-precondition', 'GEMINI_API_KEY is not set. Please run: firebase functions:config:set gemini.key="YOUR_KEY"');
    }
    return new GoogleGenAI({ apiKey });
};

// 1. Generate Research Plan
export const generateResearchPlan = functions.region(REGION).https.onCall(async (data, context) => {
    // Optional: Check auth
  ...
  `;
    ...
    try {
        ...
        if (!response.text) throw new functions.https.HttpsError('internal', "No response from Gemini");
        ...
        throw new functions.https.HttpsError('internal', error.message || 'Failed to generate plan');
    }
});

// 2. Refine Research Plan
export const refineResearchPlan = functions.region(REGION).https.onCall(async (data, context) => {
...
export const analyzeTranscripts = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  `;
    ...
    try {
        ...
        if (!response.text) throw new functions.https.HttpsError('internal', "No analysis generated");
...
export const generateProjectReport = functions.region(REGION).https.onCall(async (data, context) => {
    ...
    if (!aggregatedContent) {
        throw new functions.https.HttpsError('invalid-argument', "该项目暂无有效的访谈记录。");
    }
  ...
  `;
    ...
    try {
        ...
        if (!response.text) throw new functions.https.HttpsError('internal', "Report generation failed");
